...
do_build() {
	...
	GOOS="$GOOS" GOARCH="$GOARCH" go build -ldflags="$LDFLAGS" -o $OUTPUT/$HARNESS
      ...
      build_mac64() {
        ...
        GOARCH=amd64
        LDFLAGS="-s -w"
        OUTPUT=$CWD/build/Mac/naprrqlhp
        # GNATS=nats-streaming-server
        HARNESS=naprrqlhp
        AUDITDIFFHARNESS=napcomp
        ZIP=naprrqlhp-Mac.zip
        do_build
        #do_upx
        # do_shells
        # do_zip
      ...
      build_windows64() {
        ...
        GOARCH=amd64
        LDFLAGS="-s -w"
        OUTPUT=$CWD/build/Win64/naprrqlhp
        # GNATS=nats-streaming-server.exe
        HARNESS=naprrqlhp.exe
        AUDITDIFFHARNESS=napcomp.exe
        ZIP=naprrqlhp-Win64.zip
        do_build
        #do_upx
        # do_bats
        # do_zip
      ...
      build_linux64() {
        ...
        GOARCH=amd64
        LDFLAGS="-s -w"
        OUTPUT=$CWD/build/Linux64/naprrqlhp
        # GNATS=nats-streaming-server
        HARNESS=naprrqlhp
        AUDITDIFFHARNESS=napcomp
        ZIP=naprrqlhp-Linux64.zip
        do_build
        #do_goupx
        # do_shells
        # do_zip
